...
class PipelineConfig:
    ...
    temperature_range: List[float] = field(
        default_factory=lambda: [0.3, 0.5, 0.7, 0.85, 0.95])
    ...
    llm_evaluator_model_path: Optional[str] = None  # 如果为None，使用base_model_path
    llm_evaluator_adapter_path: Optional[str] = None  # LLM评估器的LoRA适配器路径
...
class PipelineResult:
    ...
    llm_evaluation: Optional[Dict[str, Any]] = None  # LLM评估结果
...
class LayoutGenerationPipeline:
    ...
    def _init_llm_evaluator(self, model_path: Optional[str] = None, adapter_path: Optional[str] = None):
        """
        初始化LLM评估器
        ...
        try:
            ...
            logger.info("正在初始化LLM评估器: 基座=%s, LoRA=%s",
                        llm_model_path, llm_adapter_path)
            if llm_adapter_path:
...
```"""
    ...
    def build_prompt(
        ...
        classified_templates = self.prompts_config.get(
            ...
            try:
                prompt = template.format(
                    ...
                    existing_params=json.dumps(
                        existing_params, ensure_ascii=False),
                    rooms_to_generate=json.dumps(
                        rooms_to_generate, ensure_ascii=False)
                )
                return prompt
    ...
    def _generate_with_optimization(
        ...
        return PipelineResult(
            ...
            candidates_count=sum(h.get('num_candidates', 0)
...
def create_pipeline(
    ...
    enable_llm_evaluator: bool = False,
    llm_evaluator_model_path: Optional[str] = None,
    **kwargs
...
def create_pipeline_with_qwen14b_evaluator(
    base_model_path: str = "models/Qwen2.5-VL-7B-Instruct",
    lora_adapter_path: str = "lora_model",
    qwen14b_base_path: Optional[str] = None,
    qwen14b_adapter_path: Optional[str] = None,
    llm_weight: float = 0.4,
    **kwargs
) -> LayoutGenerationPipeline:
    """
    ...
    logger.info(f"创建使用 Qwen14B 评估器的 Pipeline:")
    logger.info(f"  生成模型: {base_model_path} + {lora_adapter_path}")
    logger.info(f"  评估模型: {qwen14b_base} + {qwen14b_adapter}")
