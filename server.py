...
class EvaluationRequest(BaseModel):
    ...
    full_layout: Optional[Dict[str, List[int]]
                          ] = Field(None, description="完整布局")
...
async def generate_layout(
    ...
    if pipeline is None:
        raise HTTPException(
            status_code=400, detail="请先调用 /init_pipeline 初始化生成流程")
...
async def init_llm_evaluator(
    ...
    adapter_path: Optional[str] = Form(None, description="LoRA适配器路径（可选）"),
...
async def evaluate_layout_llm(request: LLMEvaluationRequest):
    ...
    if request.model_path and llm_evaluator is None:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"LLM评估器初始化失败: {str(e)}")
    ...
    try:
        ...
        if evaluator:
            rule_result = evaluator.evaluate(
                request.layout, request.full_layout)
            # LLM是10分制，规则是100分制，综合计算
            combined_score = 0.6 * rule_result.total_score + \
                0.4 * (result.total_score * 10)
    ...
    try:
        ...
        if llm_evaluator:
            try:
                ...
                # 综合分数（规则60% + LLM40%）
                result["combined_score"] = 0.6 * rule_result.total_score + \
                    0.4 * (llm_result.total_score * 10)
...
async def validate_layout(request: ValidationRequest):
    ...
    try:
        if request.auto_fix:
            result = rule_engine.validate_and_fix(
                request.layout, request.full_layout)
        else:
