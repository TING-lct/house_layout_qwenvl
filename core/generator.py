...
from .common import extract_json_from_text, parse_layout_json
    ...
    def generate_candidates(
        ...
        if temperature_range is None:
            ...
            if num_candidates > len(temperatures_pool):
                ...
                temperature_range = [
                    0.3 + i * step for i in range(num_candidates)]
...
def select_best_candidate(
    ...
    # 按分数排序
    best_idx = max(range(len(valid_candidates)),
                   key=lambda i: valid_candidates[i].score)
