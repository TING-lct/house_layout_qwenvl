...
def download_model(
    ...
    free_gb = get_disk_free_gb(
        models_dir if models_dir.exists() else PROJECT_ROOT)
...
def main():
    ...
    lora_14b_path = PROJECT_ROOT.parent / "qwen14b" / "Qwen2.5-14B-Instruct" / \
        "Qwen2.5-14B-Instruct" / "lora" / "train_2025-12-01-21-17-23"

    print(f"\n📎 LoRA 适配器:")
    print(f"  7B生成LoRA:  {'✅' if lora_path.exists() else '❌'} {lora_path}")
    print(
        f"  14B评估LoRA: {'✅' if lora_14b_path.exists() else '❌'} {lora_14b_path}")
